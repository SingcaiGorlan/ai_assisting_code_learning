...
import { Settings, User, Bell, Shield, Palette, Database, Cpu, Save, TestTube, RefreshCw, Key, Globe, Thermometer, CheckCircle, XCircle, AlertCircle } from 'lucide-react'
...
export default function SettingsPage() {
  ...
  // 选项卡配置
  const tabs = [
    { id: 'account' as TabType, label: '账户设置', icon: <User size={18} /> },
    { id: 'ai' as TabType, label: 'AI 配置', icon: <Cpu size={18} /> },
    { id: 'notification' as TabType, label: '通知', icon: <Bell size={18} /> },
    { id: 'privacy' as TabType, label: '隐私与安全', icon: <Shield size={18} /> },
    { id: 'appearance' as TabType, label: '外观', icon: <Palette size={18} /> },
    { id: 'storage' as TabType, label: '数据存储', icon: <Database size={18} /> },
  ]
    ...
    <div className="space-y-4">
      <div className="bg-[#252526] border border-[#3c3c3c] rounded-md">
        <div className="px-5 py-4 border-b border-[#3c3c3c] flex items-center gap-3">
          <div className="text-[#007acc]"><User size={20} /></div>
          <h2 className="text-[16px] font-semibold text-white">账户信息</h2>
        </div>
        <div className="divide-y divide-[#3c3c3c]">
          <div className="px-5 py-4 flex items-center justify-between">
            <span className="text-[14px] text-white">用户名</span>
            <input 
              type="text"
              value="admin"
              readOnly
          ...
          <div className="px-5 py-4 flex items-center justify-between">
            <span className="text-[14px] text-white">邮箱</span>
            <input 
              type="email"
      ...
      <div className="p-4 bg-[#252526] border border-[#3c3c3c] rounded-md">
        <AlertCircle className="w-5 h-5 text-[#007acc] mb-2" />
  ...
  return (
    <div className="h-full bg-[#1e1e1e] overflow-y-auto custom-scrollbar">
      ...
      <div className="p-6 max-w-4xl mx-auto">
        ...
        {/* 选项卡导航 */}
        <div className="mb-6 border-b border-[#3c3c3c]">
          <div className="flex gap-1">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-2 px-4 py-3 text-[13px] font-medium transition-colors border-b-2 ${
                  activeTab === tab.id
